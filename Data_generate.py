...
def gen_Customer_CSV(rowNum):
    ...
    csvwriter.writerow(["CustomerID","FirstName","LastName","Address","City","State","DayOfBirth","Gender","ModifiedDate"])
    for i in range(1,rowNum+1): 
        ...
        csvwriter.writerow([i,firstname,lastname,faker.street_address(),faker.city(),faker.state(),
...
def crawl_Location_CSV():
    url = "http://www.nationsonline.org/oneworld/US-states-by-area.htm"
    
    r = requests.get(url)
    dfs = pd.read_html(r.text)
    df = dfs[2][["State","Census Region"]]
    df.columns = ["State", "Division"]
    time_stamp = datetime.datetime.now().strftime("%Y_%m_%d-%I_%M_%S_%p")
    df.to_csv(f'LocationData-{time_stamp}.csv', index=False)
    print("Crawl location data: Done!")
